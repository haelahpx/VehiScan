...
def process_uploaded_video(page, video_path, image_ref, status_text, download_button, output_path):
    global cap, stop_processing, frame_width, frame_height
    stop_processing = False
    writer = None
    upload_state.reset()
    ...
    try:
        ...
        with update_lock:
            status_text.value = f"Processing complete! Saved to {output_path}"
            download_button.disabled = False
            download_button.data = output_path
            page.update()
...
def main(page: ft.Page):
    ...
    # Shared state for output folder
    output_folder = ft.Ref[str]()

    # Folder picker for selecting output directory
    def handle_folder_result(e):
        output_folder.current = e.path if e.path else os.getcwd()
        page.snack_bar = ft.SnackBar(ft.Text(f"Output folder set to: {output_folder.current}"))
        page.snack_bar.open = True
        page.update()
        ...
        on_result=lambda e: (
            threading.Thread(
                ...
                args=(
                    ...
                    download_button,
                    os.path.join(
                        output_folder.current or os.getcwd(),
                        f"processed_video_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
                    )
                ),
                daemon=True
    ...
    download_button = ft.ElevatedButton(
        ...
        on_click=lambda e: page.show_dialog(
            ft.AlertDialog(
                title=ft.Text("Processed Video"),
                content=ft.Text(f"Video saved at: {download_button.data}\nOpen file location?"),
                actions=[
                    ft.TextButton("Open Folder", on_click=lambda e: os.startfile(os.path.dirname(download_button.data))),
                    ft.TextButton("Open File", on_click=lambda e: os.startfile(download_button.data)),
        ...
        if page_index == 0:
            ...
            page.controls.append(
                ft.Column([
                    ...
                    ft.Row([upload_button, select_folder_button], alignment=ft.MainAxisAlignment.CENTER),
                    upload_image,
                    status_text,
                    download_button
                ])
